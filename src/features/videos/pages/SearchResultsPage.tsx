...
import { Link, useNavigate, useSearchParams } from "../../../shared/components/Navigation";
import { Search, UserRound, Video, MessageSquare } from "lucide-react";
import { searchContent, SearchResults, UserSummary } from "../../../shared/services/api";
...
export function SearchResultsPage() {
  ...
  useEffect(() => {
    ...
    try {
      const parsed = new URL(query, typeof window !== "undefined" ? window.location.origin : "http://localhost:3000");
      if (
        parsed.origin === (typeof window !== "undefined" ? window.location.origin : "http://localhost:3000") &&
  ...
  const visibleTotal =
    activeFilter === "all"
      ? total
      : ((results.counts as Record<string, number>)?.[activeFilter] ?? 0);
    ...
    <div className="page-container search-results-page">
      ...
      <div className="search-filters" role="tablist" aria-label="Search result filters">
        {filters.map((filter) => (
          ...
          >
            {filter.label}
            {!isLoading && <span>{(results.counts as Record<string, number>)?.[filter.value] || 0}</span>}
          </button>
        ))}
      </div>
      ...
      {showPeople && results.people && results.people.length > 0 && (
        <section className="search-section" aria-labelledby="people-results-heading">
          ...
          <div className="search-people-grid">
            {results.people.map((person: UserSummary) => (
      ...
      {showVideos && results.videos && results.videos.length > 0 && (
