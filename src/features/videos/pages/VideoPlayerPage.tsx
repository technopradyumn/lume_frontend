...
import { useParams, useNavigate, Link } from "../../../shared/components/Navigation";
import {
  getVideoById,
  incrementVideoViews,
  getComments,
...
export function VideoPlayerPage() {
  ...
  const loadVideoData = async () => {
    ...
    try {
      ...
      setComments(coms || []);
      setRecommended(
        (allVids || []).filter((v: VideoItem) => v?._id !== videoId).slice(0, 6),
      );
      setSavedVideo(savedVideos.some((saved: VideoItem) => saved?._id === videoId));
    } catch (err: any) {
      ...
      const owner = current.owner
        ? {
            ...
            subscribersCount: Math.max(
              0,
              (current.owner.subscribersCount || 0) + (nextSubscribed ? 1 : -1),
            ),
          }
        : undefined;
      return {
      ...
      current.map((comment) =>
        comment._id === commentId
          ? {
              ...
              likesCount: Math.max(0, (comment.likesCount || 0) + (comment.isLiked ? -1 : 1)),
            }
          : comment,
      ),
    );
  ...
  const handleAddComment = async (vid: string, content: string) => {
    const optimistic: CommentItem = {
      _id: `pending-${Date.now()}`,
      content,
      owner: (user as UserSummary) || {
        _id: "me",
        fullName: "You",
        username: "you",
      },
