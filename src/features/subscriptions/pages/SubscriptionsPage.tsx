...
import { getSubscribedChannels, getVideos, UserSummary, VideoItem } from "../../../shared/services/api";
...
export function SubscriptionsPage() {
  const { user } = useAuth();
  const [channels, setChannels] = useState<UserSummary[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      const [chs, vids] = await Promise.all([
        user?._id ? getSubscribedChannels(user._id).catch(() => []) : Promise.resolve([]),
        getVideos().catch(() => []),
      ]);
      setChannels(chs || []);
          ...
          {channels.map((ch, i) => (
            ...
            >
              ...
              <span style={{ fontSize: "10px", color: "var(--text-tertiary)" }}>
                {formatViews(ch.subscribersCount || 0)} subs
              </span>
            </Link>
          ))}
        </div>
