...
import { getNotifications, markNotificationsAsRead, NotificationItem } from "../../../shared/services/api";
          ...
          return (
            <article key={item._id} className="notification-page__item">
              {profilePath ? (
                <Link
                  to={profilePath}
                  className="notification-page__profile"
                  aria-label={`Open ${item.sender?.fullName || item.sender?.username}'s profile`}
                >
