...
export function DashboardPage() {
  ...
  const handleUpload = async (videoData: {
    ...
    if (newVideo) {
      ...
      setStats((current) =>
        current ? { ...current, totalVideos: (current.totalVideos || 0) + 1 } : current,
      );
    }
  };
        ...
        {isLoading || statsUnavailable
          ? Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="stat-card"
                aria-label="Loading channel metric"
              >
                <div
                  className="shimmer"
                  style={{
                ...
                <div
                  className="shimmer"
                  style={{ width: 84, height: 28, marginTop: 14 }}
                />
                <div
                  className="shimmer"
                  style={{ width: 108, height: 14, marginTop: 8 }}
                />
              </div>
            ))
